...
- [x] Run `0_distributions.ipynb` to create the scm configs.
- [x] Create a prior dataset by running from `cd gtfm/gtfm; python generate.py prior_generate=tabicl_v2 dataset_name=dev`
	- `prior_generate=tabicl_v2`  uses some pre-defined `scm/` configs
		- [x] Now run them without selecting the `scm/` configs, but defining scm parameters in the `tabicl_v2.yaml` directly
- [x] Understand why the number of dataset generated does not influence the time for training: "Datasets are re-used during training." <- understand what and where this happens in the codebase
- [x] Finish a dummy training run, if it takes too long: 
